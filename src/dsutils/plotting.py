...
def plot_err(xdata, ydata, err, err_neg=None, 
             xlabel='', ylabel='', alpha=0.3, color=None):
    """Plot lines with shaded error bounds.
    ...
    ydata : |ndarray|
        Y vaules of points to plot.  Should be of size ``(Nsamples,...)``.
    err : |ndarray|
        Error of each point to plot.  Should be same size as ``ydata``.
    err_neg : |ndarray| or None
        Error in the negative direction.  Should be same size as ``ydata``.
        Specify ``err`` and ``err_neg`` to define the positive and negative
        error, respectively.  Default is to use err for both pos and neg err.
    xlabel : str
        Label for the x axis. Default is no x axis label.
    ylabel : str
        Label for the y axis.  Default is no y axis label.
    ...
    if err_neg is not None and err_neg.ndim == 1:
        err_neg = np.expand_dims(err_neg, 1)

    # Check shapes
    if not np.all(ydata.shape == err.shape):
        raise ValueError('ydata and err must have same shape')
    if len(xdata) != ydata.shape[0]:
        raise ValueError('xdata does not match shape of ydata')
    if err_neg is None:
        err_neg = -err
    if not np.all(err.shape == err_neg.shape):
        raise ValueError('err_neg and err must have same shape')
    ...
    # Flatten if >1D
    ydata = np.reshape(ydata, (ydata.shape[0], Nd), order='F')
    err = np.reshape(err, (err.shape[0], Nd), order='F')
    err_neg = np.reshape(err_neg, (err_neg.shape[0], Nd), order='F')

    # Plot the data
    for iD in range(Nd): #for each dataset,
        next_color = get_next_color(color, iD)
        lab = get_ix_label(iD, dims)
        plt.fill_between(xdata, ydata[:,iD]+err[:,iD], 
                         ydata[:,iD]+err_neg[:,iD],
                         alpha=alpha, facecolor=next_color)
        plt.plot(xdata, ydata, color=next_color)
...
def fill_between(xdata, lb, ub, xlabel='', ylabel='', alpha=0.3, color=None):
    ...
    xdata : |ndarray|
        X values of points to plot.  Should be vector of length ``Nsamples``.
    lb : |ndarray|
        Lower Y vaules to plot.  Should be of size ``(Nsamples,...)``.
    ub : |ndarray|
        Upper Y vaules to plot.  Should be of size ``(Nsamples,...)``.
    xlabel : str
        Label for the x axis. Default is no x axis label.
    ylabel : str
        Label for the y axis.  Default is no y axis label.
